...
def find_lr(model, train_loader, test_loader, start_lr, end_lr):
    ...
    optimizer=optim.SGD(model.parameters(), lr=start_lr,
                          momentum=0.90, weight_decay=0.005)
    ...
    lr_finder.range_test(train_loader, val_loader=test_loader, end_lr=end_lr,
                         num_iter=num_iterations, step_mode="linear", diverge_th=50)
...
def train_model(model, criterion, device, train_loader, test_loader, optimizer, scheduler, EPOCHS):
    ...
    """
    miss_index = torch.where(
        (results.argmax(dim=1) == torch.tensor(test_targets).to(device)) == False)[0]
    hit_index = torch.where(
        (results.argmax(dim=1) == torch.tensor(test_targets).to(device)) == True)[0]
